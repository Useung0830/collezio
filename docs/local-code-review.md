...
- 로컬에 다운로드한 모델: 기본값 `gemma4:12b` (`ollama pull gemma4:12b`)
...
# 이미 다운로드한 비추론 모드 지원 로컬 모델로 변경한다.
...
- 직접 import한 `src` 내부 TypeScript 소스를 같은 커밋에서 최대 6개·합계 24KB까지 추가한다.
  `@/`와 상대경로를 처리하며 후보 경로 16개까지만 확인한다. 삭제된 파일은 base 커밋을 사용한다.
  추가 소스는 실행하지 않으며, 외부 패키지·동적 import·전체 호출 관계는 탐색하지 않는다.
- 저장소 전체와 모든 호출 관계를 탐색하는 에이전트가 아니다. 교차 파일 오류를 놓칠 수 있다.
- 최대 30개 변경 파일, 파일별 전체 입력 80KB, 문맥 32K 토큰, 생성 2,048토큰으로 제한한다.
- 속도를 위해 추론 모드를 끄고 스트리밍으로 수신한다. 파일별 제한 시간은 10분이며 추론 텍스트는 저장·게시하지 않는다.
- 생성 한도·문맥 초과·최종 답변 누락은 파일명과 토큰 사용량을 포함해 구분하여 기록한다.
...
현재 기본값은 속도를 우선한 Gemma 12B 비추론 모드다. 이전 PR #14의 10개 파일 시험은 약 61초였으나,
오류 누락과 오탐도 있었다. 관련 코드 문맥 추가 후의 속도는 PR과 입력 크기에 따라 달라진다.

비교를 위해 RTX 3080 12GB / RAM 64GB에서 Qwen3.8 27B의 추론 모드와 관련 코드 문맥을 시험했다.
작은 합성 테스트에서 사용자 조회 필터 누락과 최신순 정렬 오류를 모두 찾았으며 정상 코드에는 지적이 없었다.
PR #14의 대표 3개 파일에서도 오탐이 없었지만 파일당 약 5~9분, 합계 약 19분 27초가 걸렸다.
전체 10개 파일을 검증한 결과는 아니다. 이전 Gemma·Qwen3-Coder 시험과 추론·문맥 설정도 달라
개선을 모델 자체만의 효과로 단정할 수 없다. Claude와 동급으로 검증된 구성은 아니며,
실제 PR에서 놓친 문제와 오탐을 기록하며 평가한다. Qwen3.8은 PR #16의 문서 리뷰에서
추론 포함 4,096토큰 한도에 도달해 실패했고, 속도를 우선해 Gemma로 전환했다.
