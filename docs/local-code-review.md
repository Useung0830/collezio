...
- 로컬에 다운로드한 모델: 기본값 `qwen3.8:27b` (`ollama pull qwen3.8:27b`)
...
- 최대 30개 변경 파일, 파일별 전체 입력 80KB, 문맥 32K 토큰, 추론 포함 생성 4,096토큰으로 제한한다.
- 추론 모드를 활성화하고 스트리밍으로 수신한다. 파일별 제한 시간은 15분이며 추론 텍스트는 저장·게시하지 않는다.
...
RTX 3080 12GB / RAM 64GB에서 Qwen3.8 27B의 추론 모드와 관련 코드 문맥을 시험했다.
...
실제 PR에서 놓친 문제와 오탐을 기록하며 평가한다. 파일 수에 따라 전체 리뷰에 상당한 시간이 걸릴 수 있다.
